//2018/Jan/04
//This file is modified form the file which is provided from OMRON.
//License is below.
...
static int okao_expr[5];
static int okao_age;
static int okao_gender;
...
int okao(int com_num, int boundrate)
{
    ...
    serialStat.com_num = com_num;
    serialStat.BaudRate = boundrate;        /* Default Baudrate = 9600 */
    ...
    do {
        ...
        sizeRange.hdMaxSize = HAND_SIZE_RANGE_MAX_DEFAULT; sizeRange.dtMinSize = FACE_SIZE_RANGE_MIN_DEFAULT;
...
//int main(int argc, char *argv[])
int okao_init(int com_num, int boundrate)
{
    INT32 ret = 0;  /* Return code */
    INT32 inRate;
    int listBaudRate[] = {
...
int okao_exec()
{
    ...
    execFlag = HVC_ACTIV_BODY_DETECTION | HVC_ACTIV_HAND_DETECTION | HVC_ACTIV_FACE_DETECTION | HVC_ACTIV_FACE_DIRECTION |
                HVC_ACTIV_AGE_ESTIMATION | HVC_ACTIV_GENDER_ESTIMATION | HVC_ACTIV_GAZE_ESTIMATION | HVC_ACTIV_BLINK_ESTIMATION |
                HVC_ACTIV_EXPRESSION_ESTIMATION | HVC_ACTIV_FACE_RECOGNITION;
    imageNo = HVC_EXECUTE_IMAGE_NONE; /* HVC_EXECUTE_IMAGE_NONE; */
        ...
        for(int i = 0; i < pHVCResult->fdResult.num; i++){
            ...
            if(pHVCResult->executedFunc & HVC_ACTIV_AGE_ESTIMATION){
                ...
                if(-128 == pHVCResult->fdResult.fcResult[i].ageResult.age){
                    ...
                    sprintf_s(&pStr[strlen(pStr)], LOGBUFFERSIZE-strlen(pStr), "\n      Age\t\tAge:%d Confidence:%d",
                                pHVCResult->fdResult.fcResult[i].ageResult.age, pHVCResult->fdResult.fcResult[i].ageResult.confidence);
                    okao_age = pHVCResult->fdResult.fcResult[i].ageResult.age;
                }
            }
            if(pHVCResult->executedFunc & HVC_ACTIV_GENDER_ESTIMATION){
                /* Gender */
            ...
            if(pHVCResult->executedFunc & HVC_ACTIV_EXPRESSION_ESTIMATION){
                ...
                if(-128 == pHVCResult->fdResult.fcResult[i].expressionResult.score[0]){
                    ...
                    for(int idx = 0;idx < 5;idx++)
                    {
                        okao_expr[idx] = pHVCResult->fdResult.fcResult[i].expressionResult.score[idx];
                    }
                }
            }
        }
...
int okao_getHappiness()
{
    return okao_expr[1];
}

int okao_getSurprise()
{
    return okao_expr[2];
}
...
BOOST_PYTHON_MODULE(libokao)
{
    using namespace boost::python;
    def("okao", okao);
    def("okao_init",            okao_init);
    def("okao_exec",            okao_exec);
    def("okao_close",           okao_close);

    def("okao_getNeutral",      okao_getNeutral);
    def("okao_getHappiness",    okao_getHappiness);
    def("okao_getSurprise",     okao_getSurprise);
    def("okao_getAnger",        okao_getAnger);
    def("okao_getSadness",      okao_getSadness);
    def("okao_getAge",          okao_getAge);
    def("okao_getGender",       okao_getGender);
}
